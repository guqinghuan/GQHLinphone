...
public class AccountPreferencesFragment extends PreferencesListFragment {
	...
	OnPreferenceChangeListener passwordChangedListener = new OnPreferenceChangeListener() {
		...
		public boolean onPreferenceChange(Preference preference, Object newValue) {
			if(isEditTextEmpty(newValue.toString())) return false;
			//+add,guqinghuan.yuanda,20160629,password use md5
			if(com.yuanda.linphone.util.YDFeatureOptions.PASSWORD_MD5_STORE_SUPPORT
					&& (preference instanceof EditTextPreference))
			{
				String md5Pwd = newValue.toString();
				if(!((EditTextPreference)preference).getText().equals(newValue.toString()))
				{
					md5Pwd = com.yuanda.linphone.util.YDMd5Secure.getMD5Str(md5Pwd);
				}
				if (isNewAccount) {
